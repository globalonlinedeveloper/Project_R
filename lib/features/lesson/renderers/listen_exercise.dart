...
/// it from a word bank (the Build shell + a [ListenAudioControls] row). Design:
...
/// [AudioHandle]; the shared [ListenAudioControls] shows a non-blocking hint on a
/// handle throw and the bank + Check stay usable. When no audio is available the
...
///
/// NOTE: this word-bank variant needs a tokenizer + decoy source (SPEC §4,
/// `[owner/engine]`); the runner currently serves the tokenizer-free
/// type-what-you-hear Listen (audio + typed). This widget is the ready target
/// for the word-bank Listen once that engine lands.
...
class _ListenExerciseState extends State<ListenExercise> {
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      ...
      children: <Widget>[
        ListenAudioControls(audio: widget.audio, reduceMotion: widget.reduceMotion),
