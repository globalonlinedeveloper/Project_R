...
Turns compact Opus-authored story specs into schema-correct passage(kind=story)
...
def emit(spec):
    ...
    pid = f"passage_en_{ll}_story_{k}"
    tref = f"passagetitle_en_{ll}_story_{k}"
    eref = f"passageexpl_en_{ll}_story_{k}"
    ...
    for i, (text, expl) in enumerate(spec["lines"], 1):
        sid = f"sentence_en_{ll}_story_{k}_{i:02d}"
    ...
    for j, c in enumerate(spec["checks"], 1):
        iid = f"item_en_{ll}_story_{k}_chk_{j}"
        pref = f"prompt_en_{ll}_story_{k}_chk_{j}"
        opts = []
        correct_text = None
        ...
        chk_refs.append(iid)
    P.append({"passage_id": pid, "locale": "en", "kind": "story",
              "title_ref": tref, "cefr_level": lvl, "theme": spec["theme"],
              "collection_id": f"collection_en_{ll}_stories_1",
...
if __name__ == "__main__":
    from stories_specs import STORIES
    draft = build(STORIES)
    out = sys.argv[1] if len(sys.argv) > 1 else "stories_wave_draft.json"
    json.dump(draft, open(out, "w", encoding="utf-8"), ensure_ascii=False, indent=2)
    c = {t: len(v) for t, v in draft["tables"].items()}
    print(f"emitted {len(STORIES)} stories -> {out}  rows={c}  total={sum(c.values())}")
